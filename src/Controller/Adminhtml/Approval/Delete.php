...
class Delete extends Action implements HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        try {
            $success = $this->approvalManager->deleteProposal($proposalId);

            if ($success) {
                $this->messageManager->addSuccessMessage(__('Proposal has been deleted.'));
            } else {
                $this->messageManager->addErrorMessage(__('Failed to delete proposal.'));
            }
        } catch (\Exception $e) {
